...
class siMLPe(nn.Module):
    def __init__(self, config):
        ...
        if self.temporal_fc_in:
            self.motion_fc_in = nn.Linear(self.config.motion.bam_input_length_dct, self.config.motion.bam_input_length_dct)
        else:
            self.motion_fc_in = nn.Linear(self.config.motion.dim, self.config.motion.dim)
        if self.temporal_fc_out:
            self.motion_fc_out = nn.Linear(self.config.motion.bam_input_length_dct, self.config.motion.bam_input_length_dct)
        else:
